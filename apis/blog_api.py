# Arhivo de guia para basarnos. 
# Se comentara cada linea para usarlo como guia desde este proyecto, como documentacion 
...
# app es el objeto principal de la API
# Se encarga de manejar las peticiones y respuestas
# Se inicializa con información básica sobre la API para la documentacion
# el nombre que pongas aca sera el decorador que usas para crear el decorador. ejm @Nombre.get
app = FastAPI(
    title="Blog API",
    description="Una API realista para gestión de posts y usuarios",
    version="1.0.0"
)
#Pydantic es una librería de Python que permite crear clases (modelos) para validar y estructurar datos fácilmente.
#Se usa mucho con FastAPI porque ayuda a definir cómo deben ser los datos que recibe o devuelve tu API (por ejemplo, un usuario, un post, etc.).
#Garantiza que los datos tengan el tipo correcto
# Modelos de datos usando Pydantic

#BaseModel es una clase de Pydantic. Al heredar de ella, tu clase obtiene la capacidad de validar y convertir datos automáticamente.
class UserBase(BaseModel):
    # Field(...) permite agregar validaciones y descripciones (por ejemplo, min_length=3).
    username: str = Field(..., min_length=3, max_length=50, description="Nombre de usuario único")
    email: str = Field(..., description="Email del usuario")
    full_name: Optional[str] = Field(None, max_length=100, description="Nombre completo")

# UserCreate(UserBase): Hereda de UserBase y agrega el campo password (para crear un usuario).
class UserCreate(UserBase):
    password: str = Field(..., min_length=6, description="Contraseña del usuario")

#User(UserBase): Hereda de UserBase y agrega id, created_at, is_active (para representar un usuario completo en la base de datos o la respuesta).
class User(UserBase):
    id: str
    created_at: datetime
    is_active: bool = True
